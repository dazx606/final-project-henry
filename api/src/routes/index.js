...
router.get('/cars/:locationId', async (req, res, next) => {
  ...
  const infoFormatter = (info) => {
    if (!info) return res.status(404).json("No corresponding car found!");
    let filterdCars = info.carModels.map(c => {
      const existingRents = [];
  ...
  try {
    if (brand && category) {
      ...
      return infoFormatter(locationCarModels);
    }
    if (brand) {
      const locationCarModels = await Location.findByPk(
        locationId,
          ...
          include: [
            {
              model: CarModel, include: [
                ...
                { model: OptionalEquipment, attributes: ['name'], through: { attributes: [] } },
                {
                  model: IndividualCar, attributes: ['id']
                  , include: [{ model: RentOrder, attributes: ['startingDate', 'endingDate'] }]
                }
              ]
            },
          ]
